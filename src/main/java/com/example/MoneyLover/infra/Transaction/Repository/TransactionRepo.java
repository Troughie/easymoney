...
//    @Query("select  new com.example.MoneyLover.infra.Transaction.Dto.Transaction_Response(t.id,t.amount,t.remind,t.exclude,t.notes,t.category,t.date,t.user) from Transaction t where t.user=?1")
//    List<Transaction_Response> getAll(User user);
//
//    @Query("select  new com.example.MoneyLover.infra.Transaction.Dto.Transaction_Response(t.id,t.amount,t.remind,t.exclude,t.notes,t.category,t.date,t.user) from Transaction t where t.wallet.id=?1 and t.date between ?2 and ?3")
//    List<Transaction_Response> getAllInMonth(String wallet_id, LocalDate start , LocalDate end);
//
//    @Query("select  new com.example.MoneyLover.infra.Transaction.Dto.Transaction_Response(t.id,t.amount,t.remind,t.exclude,t.notes,t.category,t.date,t.user) from Transaction t where t.user=?1 and t.remind=true")
